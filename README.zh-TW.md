...
一則可疑連結會引出三種不同的問題，各自需要不同的工作：

- **這個說法是真是假？** —— 內容查核。
- **它怎麼擴散、誰在推？** —— 敘事與協同行為研究。
- **這個 URL 背後是誰、哪些網域屬於同一個操作者？** —— 基礎設施歸因。

kwara 只回答第三種，而且產出設計成可以交給前兩種工作接手。它從你給的起點出發——案件裡的 URL，或一批待篩選的候選母體——蒐集背後的基礎設施訊號，組裝成一份對方不必信任你就能自行驗證的封包。

### 與其他工具的分野

| | 它做什麼 | kwara 補上什麼 |
|---|---|---|
| WHOIS／ASN 查詢 | 單域單次查詢，隱私代理一遮就斷 | **跨域關聯**——多域的 pixel／憑證／參數／header 是否互相連得起來 |
| urlscan.io | 單一 URL 的基礎設施掃描，資料留在別人的平台上 | **本機案件庫**——多次擷取可跨時間比對，匯出的封包離線就能驗證 |
| Hunchly | 瀏覽器擴充，對你造訪過的每個頁面做雜湊、時間戳與完整擷取，組成案件檔 | **自己動手的無頭蒐集**——ads.txt、WHOIS、TLS、轉址鏈——並依共用訊號把網域分群，而不是記錄你瀏覽過哪裡 |
| Maltego、SpiderFoot | 廣度大的通用 OSINT，有龐大的 transform／模組生態 | **特化於廣告生態**，另有單案件生命週期、稽核軌跡，以及對匯出封包的 RFC 3161 時間戳 |
