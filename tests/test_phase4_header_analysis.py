...
QSH 2026-04-28 patterns are the positive controls:
  - picread.net leaks `x-server-hosted: Malaysia Cloud Pte Ltd`
...
def test_per_domain_constants_returns_stable_value_across_hops():
    """picread.net seen 3x with the same x-server-hosted = constant."""
    conn = _fresh_db()
    case_id = _seed_case(conn)
    for _ in range(3):
        _seed_hop(conn, case_id, "http://picread.net/article/x", [
            ["Server", "nginx"],
    ...
    assert "picread.net" in out
    assert out["picread.net"]["server"] == "nginx"
    assert out["picread.net"]["x-server-hosted"] == "Malaysia Cloud Pte Ltd"
    # Volatile headers (Date) excluded
    assert "date" not in out["picread.net"]
...
def test_cross_domain_shared_template_picks_up_shared_x_powered_by():
    ...
    for d in ("picelse.com", "luckyelse.com", "maimai.pro"):
        _seed_hop(conn, case_id, f"http://{d}/article", [
            ["Server", "Apache"],
            ["x-powered-by", fake],
        ])
    ...
    assert set(matches[0]["domains"]) == {"picelse.com", "luckyelse.com", "maimai.pro"}
...
def test_detect_fake_apache_25_and_openssl_112():
    """The QSH picread fingerprint — Apache 2.5.1 + OpenSSL 1.1.2e are
...
def test_cookie_origin_leak_when_domain_attr_differs():
    ...
    assert any(
        r["response_domain"] == "picread.net" and r["cookie_domain"] == "realorigin.example"
...
def test_cookie_shared_template_across_domains():
    ...
    conn = _fresh_db()
    case_id = _seed_case(conn)
    for d in ("picelse.com", "luckyelse.com"):
    ...
    assert set(sharing[0]["domains"]) == {"picelse.com", "luckyelse.com"}
