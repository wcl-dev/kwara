...
def test_matrix_uses_uid_when_utm_term_absent():
    """picread.net wrapper case: original URL has uid=, no utm_term."""
    conn = _make_db()
    case_id = _make_case(conn)
    _add(conn, case_id, "Alice", "2026-01-01 10:00", "https://picread.net/article/1?uid=638")
    _add(conn, case_id, "Bob",   "2026-01-01 11:00", "https://picread.net/article/1?uid=638")
    m = account_content_matrix(conn, case_id)
