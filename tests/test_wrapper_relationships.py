...
def test_unscanned_url_excluded():
    conn = _make_db()
    case_id = _make_case(conn)
    _add_unscanned(conn, case_id, "https://picread.net/article/1")
    assert wrapper_relationships(conn, case_id) == []
...
def test_same_domain_redirect_excluded():
    ...
    _add_scanned(conn, case_id,
                 ...
                 "https://example.com/articles/full-path")
    assert wrapper_relationships(conn, case_id) == []


def test_single_wrapper_to_target_relationship():
    """One URL: picread.net → maimai.pro should appear as one row."""
    conn = _make_db()
    case_id = _make_case(conn)
    _add_scanned(conn, case_id,
                 "https://picread.net/article/277290?uid=638",
                 "https://maimai.pro/article/277290?utm_term=638")
    out = wrapper_relationships(conn, case_id)
    assert len(out) == 1
    r = out[0]
    assert r["original_domain"] == "picread.net"
    assert r["final_domain"] == "maimai.pro"
...
def test_multiple_urls_same_wrapper_aggregate():
    ...
    for i in range(5):
        _add_scanned(
            conn, case_id,
            f"https://picread.net/article/{i}?uid={i}",
            f"https://maimai.pro/article/{i}?utm_term={i}",
        )
    out = wrapper_relationships(conn, case_id)
    assert len(out) == 1
...
def test_one_wrapper_to_multiple_targets_makes_multiple_rows():
    """If picread.net redirects some URLs to A and others to B, we get 2 rows."""
    conn = _make_db()
    case_id = _make_case(conn)
    for i in range(3):
        _add_scanned(conn, case_id, f"https://picread.net/a/{i}", "https://maimai.pro/")
    for i in range(2):
        _add_scanned(conn, case_id, f"https://picread.net/b/{i}", "https://other.example/")

    out = wrapper_relationships(conn, case_id)
    assert len(out) == 2
    targets = {(r["original_domain"], r["final_domain"]) for r in out}
    assert targets == {
        ("picread.net", "maimai.pro"),
        ("picread.net", "other.example"),
    }
