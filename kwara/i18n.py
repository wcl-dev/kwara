...
_EN: dict[str, str] = {
    ...
    "clusters.tracking_ids_caption": "The same Meta Pixel / GA / GTM / Google Ads / TikTok ID embedded across multiple landing domains. A pixel ID belongs to exactly one ad/analytics account — when it appears on multiple domains, those domains are tracked from one operator's account. Strongest cross-domain attribution short of seizure, and immune to Cloudflare/CDN fronting. Requires page snapshots (run them in **Preserve → Page**).",
    "clusters.no_tracking_ids": "No cross-domain tracking ID matches. Capture page snapshots in **Preserve → Page** to populate.",
    ...
    "clusters.no_endpoints": "No cross-domain third-party endpoints detected. Capture page snapshots in **Preserve → Page** to populate.",
    ...
    "prov.no_ad_tracking": "No identified ad/tracking platforms detected. Add URLs with tracking parameters, or capture page snapshots in **Preserve → Page**.",
    ...
    "prov.no_ads_txt": "No ads.txt fetched yet. Click the button above, or run snapshots in **Preserve → Page** (ads.txt is fetched alongside).",
    ...
    "insights.bullet_cloaking": "**Active evasion — cloaking:** {n} URL(s) serve different content with vs without tracking params. The operator wrote logic to vary behaviour by visitor type — see the **Cloaking** sub-tab.",
    "insights.bullet_fake_version": "**Fabricated server fingerprint:** {n} header value(s) claim a version that never shipped (e.g. `{header}: {value}` on {domain}). Faking these is an intentional anti-forensic move — see the **Headers** sub-tab.",
    "insights.bullet_header_template": "**Shared server template:** {n} header value(s) appear across multiple domains (top: `{header}: {value}` on {domains} domains). Same operator template — origin headers survive CDN fronting. See the **Headers** sub-tab.",
    "insights.bullet_opsec_strong": "**Strong UA-gating ({n} domain(s)):** {domains} block lightweight fetches but render in real Chromium — a WAF-template signal independent of GA4/TLS. See the **OPSEC** sub-tab.",
...
_ZH: dict[str, str] = {
    ...
    "clusters.no_wrappers": "未發現包裝器關係。請先在「掃描」分頁執行掃描。",
    ...
    "clusters.tracking_ids_caption": "Meta Pixel / GA / GTM / Google Ads / TikTok 等追蹤碼**內嵌**在多個落地網域的 HTML。Pixel ID 等同於操作者的廣告／分析帳號代號——同一個 ID 出現在多個網域 = **這些網域由同一個帳號在追蹤**。短缺扣押之外最強的跨網域歸屬訊號，且不受 Cloudflare/CDN 影響（因為訊號活在頁面本身）。需先在「證據保全 → 頁面證據」執行 snapshot 截圖。",
    "clusters.no_tracking_ids": "未發現跨網域共用的追蹤碼。請先到「證據保全 → 頁面證據」執行截圖。",
    ...
    "clusters.no_endpoints": "未偵測到跨網域第三方 endpoint。請先在「證據保全 → 頁面證據」執行截圖。",
    ...
    "prov.no_ad_tracking": "未偵測到可辨識的廣告／追蹤平台。請新增帶追蹤參數的 URL，或在「證據保全 → 頁面證據」執行截圖。",
    ...
    "prov.no_ads_txt": "尚未抓取任何 ads.txt。請點上方按鈕,或在「證據保全 → 頁面證據」執行截圖(ads.txt 會一併抓取)。",
    ...
    "insights.bullet_cloaking": "**主動防偵測——cloaking：** {n} 條 URL 在「帶／不帶追蹤參數」時回傳不同內容。操作者刻意寫了依訪客類型區別行為的邏輯——詳見 **Cloaking** 子分頁。",
    "insights.bullet_fake_version": "**偽造伺服器指紋：** {n} 個 header 值宣稱了從未發行的版本（例如 {domain} 上的 `{header}: {value}`）。造假即刻意反鑑識——詳見 **Headers** 子分頁。",
    "insights.bullet_header_template": "**共用伺服器模板：** {n} 個 header 值跨多個網域出現（最強者：`{header}: {value}` 出現在 {domains} 個網域）。同一操作者模板——origin header 不被 CDN 擦除。詳見 **Headers** 子分頁。",
    "insights.bullet_opsec_strong": "**強 UA 阻擋（{n} 個網域）：** {domains} 擋掉輕量抓取但在真實 Chromium 正常渲染——獨立於 GA4／TLS 的 WAF 模板訊號。詳見 **OPSEC** 子分頁。",
