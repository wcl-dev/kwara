...
_EN: dict[str, str] = {
    ...
    "tab.account_patterns": "Account Patterns",
    "account_patterns.help": "Exploratory pivot views of poster × content. Descriptive only — no thresholds, no automatic 'coordinated' flags. Read these as raw distributions to inform your own judgement; calibration via auto-flagging requires multiple operator datasets we don't yet have.",
    "account_patterns.matrix": "Account × Content Matrix",
    "account_patterns.matrix_caption": "Rows: poster accounts. Columns: content IDs (utm_term / uid). Cells: post counts. Use this to spot accounts that share content (rows that light up on the same column).",
    "account_patterns.no_data": "No content IDs detected. Posts must contain a URL with utm_term, uid, utm_id or campaign_id parameter.",
...
_ZH: dict[str, str] = {
    ...
    "tab.insights": "分析洞察",
    "tab.account_patterns": "帳號樣態",
    "account_patterns.help": "貼文者 × 內容的探索式 pivot 檢視。**純描述、無閾值、不自動判定協同**——把這些訊號當作原始分布來輔助判斷即可，自動旗標需要多個操作者的對照資料才能校準（目前還沒有）。",
    "account_patterns.matrix": "帳號 × 內容 矩陣",
    "account_patterns.matrix_caption": "列為帳號、欄為內容 ID（utm_term / uid）、格為貼文次數。看哪些帳號**在同一欄上一起亮燈**——那就是它們共同推同一支內容。",
    "account_patterns.no_data": "未偵測到內容 ID。貼文 URL 需包含 utm_term、uid、utm_id 或 campaign_id 之一。",
