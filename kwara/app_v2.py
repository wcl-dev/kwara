"""kwara — UI-redesign prototype entry point (st.navigation).

Run side-by-side with the production app:

    streamlit run app_v2.py        # this prototype (left-rail navigation)
    streamlit run app.py           # current shipping UI (top tabs)

What this prototype demonstrates (vs. the 7-top-tab + nested-sub-tab app):
  • a real LEFT-RAIL navigation (st.navigation) — no more tabs-in-tabs
  • an Overview verdict page as the default landing surface (VirusTotal +
    Grafana fusion)
  • an Operator Graph page (Maltego / VT-Relations) drawn from the existing
    shared-signal clustering engines
  • Analysis reorganised by QUESTION ("is this one operator / are they
    hiding / who profits") instead of by detection MECHANISM

It writes nothing new and reuses every existing view render() function, so
the two entry points share one database and one codebase.
"""
import os
import sys
...
# ---------------------------------------------------------------------------
# Sidebar — case selection + language (the navigation rail is auto-added by
# st.navigation below the title).
# ---------------------------------------------------------------------------
with st.sidebar:
    st.title("kwara")

    lang_options = list(LANGUAGES.keys())
    cur = lang_options.index(get_lang()) if get_lang() in lang_options else 0
    sel_lang = st.selectbox(
        "Language", lang_options, index=cur,
        format_func=lambda x: LANGUAGES[x], key="lang_sel_v2",
    ...
    if cases:
        ...
        selected = st.selectbox("作用中案件 Active case", list(case_options.keys()))
        current_case_id = case_options[selected]
    else:
        st.info("尚無案件 — 用現有的 app.py 建立，或之後在此原型加上建案 UI。")
