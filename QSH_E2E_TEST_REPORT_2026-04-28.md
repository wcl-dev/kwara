...
|---|---|---|---|
| hubsite.example | 73 | hubsite.example | 1 |
| crawlerlanding.example | 21 | visitorlanding.example | 2 |
| crawlerlanding2.example | 2 | visitorlanding.example | 2 |
| satellitesite.example | 4 | satellitesite.example | 1 |

→ 終點分布：**hubsite.example 73 / visitorlanding.example 23 / satellitesite.example 4**。

### 4.2 crawlerlanding → visitorlanding 換殼模式（wrapper_relationships）

`clustering_url.wrapper_relationships()` 找出 2 條換殼路線（記憶中已記錄的 crawlerlanding 包裝模式）：

```
crawlerlanding.example  → visitorlanding.example    (21 URL,   2-hop redirect)
crawlerlanding2.example  → visitorlanding.example    ( 2 URL,   2-hop redirect)
```

換殼保留分析師代碼：crawlerlanding URL 上的 `?uid=638/606/798/3315/3305` → visitorlanding 上的 `utm_term=638/606/798/3315/3305`。**同一識別碼跨域名沿用，操作者誰投放哪個帳號是可追溯的。**

### 4.3 GA4 Pixel 共用（shared_tracking_ids，最關鍵發現）

```
G-T5N9K2Q7W3  → hubsite.example (73 page) + satellitesite.example (4 page)
...
→ 至少 **2 個分離的操作者集群**：
- **集群 A（hubsite + satellitesite）** — 共用 `G-T5N9K2Q7W3`
- **集群 B（visitorlanding.example，含 crawlerlanding 換殼前端）** — 自己的 GA4 ×2 + GTM
...
domains       satellitesite.example, visitorlanding.example     ← 4h50m 內同一 CA 連續發證
```
...
| google_tag_manager | html_embedded | 23 / 23 | visitorlanding.example | `GTM-T5N9K2Q` |
| generic（uid 參數） | url_param | 23 / 23 | crawlerlanding.example + crawlerlanding2.example | — |
...
| ipac.ctnsnet.com | visitorlanding + hubsite | 跨網域 sync |
...
| www.facebook.com | satellitesite + hubsite | Meta Pixel sink |
| www.temu.com | visitorlanding + hubsite | 變現對象 |

→ 即使 GA4 不共用（如 visitorlanding.example vs 集群 A），廣告供應鏈仍橫跨——獨立操作者選用了相似的供應鏈組合，或 Cocovn / Temu 為共同 OPSEC 跡象。
...
```
utm_term=145    → hubsite.example,    54 posts / 46 URL  (paid_keyword)
utm_term=8901   → hubsite.example,     9 posts /  9 URL
uid=638         → crawlerlanding.example,     9 posts /  8 URL  (user_tracking_id, generic)
utm_term=638    → visitorlanding.example,      9 posts /  8 URL  ← 同 638 在 wrapper 後變成 utm_term
```

`utm_term=638` 在 visitorlanding.example 與 `uid=638` 在 crawlerlanding.example 是**同一個分析師代碼跨換殼存活下來**的明確證據。
...
|---|---|---|---|
| hubsite.example | 73 | 73 | **9 個 FB actor** |
| visitorlanding.example | 23 | 23 | **13 個 FB actor** |
| satellitesite.example | 4 | 4 | 3 個 FB actor |

→ **visitorlanding.example 的 13 個不同 FB 帳號**（番茄小說、養生頭條、Senior Chef、故事分享、有故事的人、女人帮、天天看戲、言情小說家、言情迷、…）全部用 crawlerlanding 換殼指向同一終點：典型分身帳號 → 單一基礎設施模式。
...
|---|---|---|---|
| visitorlanding.example | 23 / 23 | 0 | 0%（無 WAF） |
| hubsite.example | 12 / 73 | 61 | **84%** |
| satellitesite.example | 0 / 4 | 4 | **100%** |

→ **集群 A（hubsite + satellitesite）**啟用 WAF 阻擋非瀏覽器 UA，**集群 B（visitorlanding.example）**未啟用——進一步支持兩者為獨立操作者；同時也驗證 Phase 3 ticket C 的 trade-off 設計：輕量路徑會被 WAF 大量阻擋、Playwright 仍是有 OPSEC 站的唯一可靠路徑。
...
| #4 Fingerprint 必須有 invocation context | ✅ 抽出的 GTM-T5N9K2Q、3 個 G-xxxxxxxxx 全是真貨、零誤判 |
...
1. **集群 A — hubsite + satellitesite**
   - 同一個 GA4 帳號 `G-T5N9K2Q7W3`
   - 阻擋非瀏覽器 UA（OPSEC 較嚴）
   - 全在 Cloudflare、Google Trust Services 簽發
   - 9 個 FB actor 投放 hubsite、3 個投放 satellitesite
   - **強推「同操作者」**

2. **集群 B — visitorlanding.example（含 crawlerlanding.* 換殼前端）**
   - 自己的 2 個 GA4 + 1 個 GTM
   - **不阻擋 UA**（OPSEC 較鬆）
   - 同樣在 Cloudflare、Google Trust Services
   - **23 筆全部走 2-hop redirect**（crawlerlanding.example → visitorlanding.example 與 crawlerlanding2.example → visitorlanding.example）
   - 13 個 FB actor 透過換殼投放
   - **強推「同操作者，但與集群 A 不同」**

3. **集群間共同點**：CA、ASN、9 個共用 DSP／sync 端點——**廣告供應鏈高度重疊**、可能委外同一行銷代理、或為產業共用堆疊；單獨不能斷定同操作者。

4. **`account_content_matrix` 的「無 content_id 跨 actor」結果**證明：操作者讓 18 個 FB 帳號各自貼不同 redacted139 ID，避免 1:1 重複落入 FB 的協同行為偵測——但所有 redacted139 ID 落到同一基礎設施仍可被 kwara 偵測。
...
**排查**：
- 同時段輕量 HTTP fetch（純 `requests.get`）對部分目標（visitorlanding 23 筆、hubsite 12 筆）回 200 OK——**網路本身正常**。
...
| 高 | **跨案件查詢**（記憶中已列） | 「`G-T5N9K2Q7W3` 出現在過去哪幾案？」 對單機分析師價值極大 |
| 中 | `account_content_matrix` 的 view 從 content_id 換成 destination_id | 目前因 redacted139 ID 不重複，所以 matrix 全部單格 — 換成「同一終點域名／GA4」會立刻看出 18 個 actor 的協同分組 |
