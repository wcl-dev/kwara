...
kwara 是一套**單機、離線**的數位證據蒐集與佐證工具，用於調查 URL 短連結濫用、網域詐騙與線上詐騙。它的定位是**跨案件的操作者基礎設施歸因引擎**——把可 pivot 的硬訊號（追蹤碼、TLS 憑證、HTTP header 指紋、cloaking 行為、ads.txt 變現帳號）累積成跨案件索引，讓同一操作者在不同案件之間再現時自動浮現。
