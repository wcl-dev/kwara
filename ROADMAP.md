...
### 候選發現（篩選漏斗）
在昂貴的證據管線之前，先用便宜的方式擴大母體：從 SSP 的 `sellers.json` 取候選發布商網域、用追蹤碼經 PublicWWW 反查埋碼網域、把 hosts／adblock／CSV 等各式封鎖清單收斂成 apex；再抓每個候選的 `/ads.txt` 與已知指紋比對、讓候選彼此自我分群（不需事先認識任何網域），並把觀測建成參照母體供 tier 判定使用。
- 篩選只能把候選升級、不能替它開脫——未命中回報 `no_match`，不是「乾淨」
- 追蹤碼反查只看得到**靜態首頁原始碼**，GTM／JS 注入的碼、只在內頁的碼、未被索引的小站都撈不到；因此它對高技術操作者最無用，null 結果不代表足跡小
- 反查會把「你在追誰」透露給第三方，屬於與 `run corroborate` 同類的揭露決定，故刻意只留在 CLI、不對 agent 開放
...
- Streamlit 介面已於 2026-08-07 移除。它顯示的判讀依據比 CLI 少（看不到 tier 的判定理由、不說明 OPSEC 為何無結果），凍結的代價從「不長新功能」變成「會誤導人」。人工檢視證據改用 `evidence browse` 投影出的網域目錄樹
...
- urlscan / Wayback 提交已由 `run corroborate` 交付，但要分析師自己記得跑；待做的是自動觸發，讓第三方留存不依賴記性

### Watchlist 與前瞻捕捉
建立在跨案件索引之上：把案件累積的強訊號抽成 watchlist，訂閱外部 feed，命中新資產即自動進入案件管線，把歸因從「事後」推向「上線即捕捉」。

四條 feed 之中，**HTML 原始碼反查已經交付**（`discover publicwww`，見上）——但目前是分析師手動發動的一次性 pivot，且受限於靜態首頁。仍待建的是 Certificate Transparency log、新註冊網域與 HTTP banner 三條，以及把四者共通的「訂閱 → 命中 → 自動進件」那一段常設化。
