...
**問題**：picread.{net,org} 看到 `?uid=` 才 302 跳轉、無 `uid` 則正常吐 25KB 內容。kwara 一律帶 tracking param 掃 → 永遠看不到 picread 服務 SEO 爬蟲那一面、誤判它為「pure wrapper」。
...
**開發效果**：自動標記「主動防偵測」訊號；證據力 **≥ GA4 共用**（後者是無意識指紋、cloaking 是有意識規避）。對 picread 這類 cloaker 不再會被騙。
...
**問題**：本次手動 `curl -D` 撈 picread 302 時，發現 3 個關鍵 OPSEC 漏洞**全部在 response headers 裡**，但 kwara 今天 `redirect_hops` 只存 `(hop_order, url, status_code, location, resolved_url, fetched_at)`，其他 30+ header 現場讀現場丟。
...
| **cross-domain header diff** | 多域名共用 server template = 同操作者 | （待跑全資料才知）若 picelse + luckyelse + maimai 都帶相同假 `x-powered-by`，cluster 證據力 = GA4 共用級別 |
