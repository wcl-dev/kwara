...
- 4 個 distinct 原始域名：picelse.com、picread.net、picread.org、luckyelse.com
...
| `uid` | 638 | generic（picread sub-campaign ID） | 9 |
...
```
picread.net (21 URL) ─┐
                     ├─ 2-hop redirect ─→  maimai.pro
picread.org ( 2 URL) ─┘
```

23 則貼文表面上連 picread，**實際全部落到 maimai.pro**。

**參數轉換被工具看到**：
- 原始 `picread.net/article/277290?uid=638`
- redirect 後 `maimai.pro/article/277290?utm_term=638#638`
- → operator 後端 PHP 把 `uid=` 改名 `utm_term=`，這個改寫規則本身是強指紋

**Visual**：兩條彎曲箭頭從 picread.net、picread.org 收斂到 maimai.pro、上方標 `?uid=` → `?utm_term=`

**Speaker Notes**：
- 真正的落地網域只有 3 個（maimai/picelse/luckyelse）不是表面的 4 個
- 換殼網域是耗材式設計 — 一個被檢舉就換 picread.com / .io / .xyz；分析師需要看穿這層
...
時間窗命中：
  luckyelse.com 與 maimai.pro 的憑證在 4h50m 內陸續簽發
...
→ **picelse + luckyelse 共用 `G-BG0P58H1GN`** = 同一個 Google Analytics 帳號 = **同一操作者**
→ maimai 自己的 GA4 + GTM = **另一個獨立操作者**

**Visual**：左半邊一個橢圓圈住 picelse + luckyelse、標記 `G-BG0P58H1GN`；右半邊另一個橢圓圈住 maimai、標記 3 個自己的 ID
...
**Visual**：3 個域名（picelse / maimai / luckyelse）在邊緣、9 個 endpoint 在中間、用線連起來；明顯密集

**Speaker Notes**：
- 即使 GA4 不共用（如 maimai vs cluster A），廣告供應鏈仍橫跨
- 兩種解讀：
...
| 終點域名 | 跨 actor 數 |
|---|---|
| picelse.com | 9 個 FB actor |
| **maimai.pro** | **13 個 FB actor**（番茄小說、養生頭條、Senior Chef、故事分享、有故事的人、女人帮、天天看戲、言情小說家、言情迷……） |
| luckyelse.com | 3 個 FB actor |

→ maimai.pro 的 13 個不同 FB 帳號全部用 picread 換殼指向同一終點 = 經典 sock-puppet → 單一基礎設施

**Visual**：一張表 — 13 個 FB 帳號頭像在左、單一 maimai logo 在右、中間 23 條收斂線

**Speaker Notes**：
- 這層直接回答 slide 3 的問題：18 個帳號分散貼到 4 個域名 → 收斂後變成 13 個帳號全部往 maimai cluster
...
**集群 A（picelse + luckyelse）**
- 共用 GA4 `G-BG0P58H1GN`
- 9 個 FB actor 投 picelse、3 個投 luckyelse
- WAF 啟用（擋非瀏覽器 UA）

**集群 B（maimai.pro，含 picread 換殼前端）**
- 自己的 2 個 GA4 + 1 個 GTM
- 13 個 FB actor 透過 picread 換殼投放
- WAF 不啟用
...
**Visual**：兩個明顯分隔的橢圓 — 左邊 A（picelse + luckyelse + 9+3 帳號）、右邊 B（maimai + picread + 13 帳號）；中間虛線連到「9 個共用 DSP」灰色框
...
|---|---|---|---|
| maimai.pro | 23/23 (100%) | 23/23 (100%) | **弱**（不擋 UA） |
| picelse.com | 12/73 (16%) | 73/73 (100%) | **中**（擋 UA、放 Chromium） |
| luckyelse.com | 0/4 (0%) | 4/4 (100%) | **強**（嚴格擋 UA） |

→ picelse + luckyelse 都擋、maimai 都放 = **獨立於 GA4 訊號**指向同樣的兩個操作者切割
...
| URL | 結果 | 內容 |
|---|---|---|
| `picread.net/article/277290`（無 uid） | HTTP 200, 25 KB | 真正的文章內容 |
| `picread.net/article/277290?uid=638`（有 uid） | HTTP 302, 0 B | → maimai.pro |
| `picread.net/`（根） | HTTP 200, 45 KB | 「世語圖說」首頁 |

→ picread 是 **conditional cloaker**：
1. 對 FB 點擊用戶（帶 uid）：302 推到 maimai 變現
2. 對 SEO 爬蟲（無 uid）：吐真內容、被 Google 收進索引
3. 對人工調查者「先清掉 tracking param」的直覺：**反而被騙**

**kwara 今天**：一律帶 tracking param 掃 → 永遠看不到 picread 服務 SEO 爬蟲那一面 → 誤判為 pure wrapper

**改善已列入 Phase 4**：cloaking detection（半天工程量）

**Visual**：picread 域名圖示分裂成三條線 — 一條到 maimai（標 FB 用戶）、一條到 Google 索引（標 SEO 爬蟲）、一條到分析師被打問號
...
**Key Points**：
- 在這次盲點排查中、用 `curl -D` 撈 picread 302 response，發現 3 個 OPSEC 漏洞：
...
- 跨案件查詢 — 「`G-BG0P58H1GN` 出現在過去哪些案件」（kwara 的 single-user 縱向追蹤優勢）
