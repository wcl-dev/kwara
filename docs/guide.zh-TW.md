...
「兩個網域提供逐字節相同的 ads.txt」是這個工具做過最強的綁定。但在 2026-08-12
之前，它同時也是**收件者只能選擇相信的主張**——kwara 算完雜湊、解析完就把位元組
丟掉，匯出包裡連 ads.txt 都沒有。

現在每個回應都留著（200、403 挑戰頁、跳轉一視同仁），而且跟著匯出包走，收件者
可以自己重算。

引用一個模板叢集之前，先看它的 `verification`：

| 判定 | 意思 |
|---|---|
| `verified` | 每個成員的位元組都在磁碟上，且仍雜湊成紀錄宣稱的值。**這個你可以拿出去講** |
| `legacy_unverifiable` | 保存機制之前抓的。是真實歷史，但沒人能再證明兩份檔案相同 |
| `body_missing` / `body_mismatch` | 留存的檔案不見了，或內容已改變 |
| `hash_disagrees` | 位元組完好，但不是推導紀錄所宣稱的那一份 |
| `truncated` | 讀取撞到大小上限；前綴雜湊不能當作整份檔案的同一性 |

**只有 `verified` 的叢集會綁定操作者群組。** 其餘出現在 `unverified_templates`
底下，附網域、宣稱的雜湊、無法驗證的原因、以及能解決它的重抓動作——看得見，
但不合併任何東西。站方若仍提供同一份檔案，重抓就能恢復；若已改檔或開始拒絕
請求，那個綁定就是真的沒了，**據實說明比含糊帶過有用**。

有兩個界線要對報告的讀者講清楚。保存讓雜湊可以重算，但**不能證明「什麼時候」
抓的**——`fetched_at` 是本機時鐘。而且兩份相同的檔案只證明擷取到的位元組相同，
不證明同一個操作者：平台自動產生的 ads.txt 模板很常見。

## 共用的 GTM 容器

GA4 資源、AdSense 發布商 ID、Meta 粉專，識別的是**某人持有的帳戶**。Google Tag
Manager 容器識別的是**標籤部署**，而代理商或 CMS 廠商完全可以合法地在不相干的
客戶之間共用同一個容器。

所以共用容器**永遠不綁定操作者群組**。它出現在 `weak_links`，層級 `相關未證實`，
帶容器 ID、所有網域、每個網域各屬哪一群，以及兩種競爭解讀——單一操作者經營兩條
變現線，或不同操作者共用代管容器。**哪一種成立，光靠資料答不出來**：追蹤碼沒有
外部參照母體，沒人說得出共用一個容器有多罕見。

GA4、AdSense、Meta 等不受影響，照舊綁定。
