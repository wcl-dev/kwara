...
| `uncertain` | 介於兩者之間 | 本機證據判不出來，**據實回報而非猜測**（見 §十五 原則 6） |
...
推導出的主張與能驗證它的原始觀察，是兩件事。kwara 一度只保存前者。

抓 ads.txt 時讀進位元組、算 SHA-256、解析出 DIRECT 帳號，然後**把位元組丟掉**；
`ads_txt_json` 只留 `raw_sha256` 與解析結果，而匯出器根本不匯出這個欄位。於是
「兩個網域提供逐字節相同的檔案」——本工具最強的綁定——變成收件者只能相信的主張。

### 兩個雜湊，不是一個

`captured_sha256` 涵蓋實際寫入的位元組；`complete_sha256` 涵蓋整個回應，**截斷時
為 NULL**。只有後者可以用來比對同一性。把前綴雜湊當作同一性，會把「前 256 KB
相同、之後不同」的兩份檔案報成同一份部署。

### 只有 `verified` 能綁定

模板叢集帶 `verification`，取自其**最弱的成員**——逐字節相同是對「兩份」檔案的
主張，一份驗不了整群就驗不了。驗證會同時檢查：內文仍在、仍雜湊成記錄值、該取得
紀錄屬於這次掃描、種類正確、且等於推導紀錄所宣稱的雜湊。少任何一項都不是通過。

驗不了的不消失，改以 `unverified_templates` 呈現，附原因與補救動作。**不會降級成
「較弱的群組」**——那等於用比較溫和的標籤保留同一個推論。

### 這一層仍然證明不了的事

保存讓雜湊可以重算，但 `fetched_at` 是本機時鐘，**不構成時間證明**；要有時間得靠
repo 外部的東西（RFC 3161 時間戳、機關的僅可附加紀錄、或推送到你無法單獨控制的
遠端）。而且兩份相同的檔案只證明擷取到的位元組相同，不證明共同持有——平台自動
產生的模板很常見。
...
GA4 資源、AdSense 發布商 ID、Meta 粉專，識別的是**帳戶**；GTM 容器識別的是**標籤
部署**，而代理商或 CMS 廠商可以合法地在不相干客戶間共用同一個容器。所以共用容器
與共同操作**相關，但不成立**。

這個判斷不是新的：分析師 2026-05-08 的報告 §3.2 就把共用 GTM 而 GA4／AdSense／
Meta 分離的情況列為「強相關但未證實同操作者」，並列出兩種解讀。**是工具太寬鬆。**

2026-08-13 在實際案件上量測，支持報告的判斷：扣掉通用程式化廣告與一個共用的商業
影音廣告廠商之後，兩群之間唯一剩下的非通用訊號就是容器本身；而其中一群的私有
樞紐被九個網域呼叫，全部屬於該群。

實作上三點：

- **永不構成 union edge**，不建立群組、不升級層級、不能把兩群間接接起來。
- **一定看得見**，於 `weak_links` 以 `相關未證實` 呈現，附兩種解讀——因為追蹤碼
  沒有外部參照母體，「共用一個容器有多罕見」目前無法回答，輸出應該說出這件事而
  不是替讀者決定。
- **不受廣度過濾**。弱訊號的規則是「出現在案件八成網域上的值算通用基礎設施」，
  對 `server: nginx` 對，對容器則相反：出現在全部網域上的容器是最值得看的那個。
  廣度照報，但不套用。

跨案件索引給它獨立的 `gtm_container` 型別，因為 `tracking_id` 的再現措辭是「同一
操作者再度出現」——那句話容器撐不起來。
